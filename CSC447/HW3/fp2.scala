...
object fp2:
  ...
  def joinTerminateRight(xs: List[String], term: String): String =
    foldRight(xs,"",(x:String, finalString:String) => x + term + finalString)
  ...
  def joinTerminateLeft(xs: List[String], term: String): String =
    (xs,term) match
      case (Nil, _) => ""
      case (y::Nil, term) => y+term
      case (y::ys, term) => foldLeft (y::ys,"",(j:String, k:String) => j+k+term)
  ...
  def firstNumGreaterThan(a: Int, xs: List[Int]): Int =
    xs match
      ...
      // put case(y::ys) on one line b/c of indentation errors while trying to make it look clean
  ...
  def firstIndexNumGreaterThan(a: Int, xs: List[Int]): Int =
    xs match
      case Nil => throw new NoSuchElementException
      case (y::ys) => if (y>=a) then 0 else 1 + firstIndexNumGreaterThan(a,ys)
