...
object argpass:
  ...
  def refint2(f: RefInt => Unit): (Int, Int, Int) =
    // TODO: Provide definition here.
    val rOne =  new RefInt (0);
    val rTwo = new RefInt (0);
    val rThree = new RefInt (0);

    f(rOne)
    f(rTwo)
    f(rThree)

    val one : Int = rOne.get()
    val two : Int = rTwo.get()
    val three : Int = rThree.get()
    (one, two, three)
  ...
  def refint4(r: RefInt, f: RefInt => Unit): Boolean =
    // TODO: Provide definition here.
    val rSelf : RefInt = new RefInt(r.get())
    f(rSelf)

    if(r.get() == rSelf.get()) then true else false
