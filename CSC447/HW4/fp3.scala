...
object fp3:
  ...
  def stringLengths(xs: List[String]): List[(String, Int)] =
    xs match
      case Nil => Nil
      case y::ys => xs.map((x:String) => (x,x.length))
  ...
  def removeDupes1[X](xs: List[X]): List[X] =
    xs match
      ...
      case y::Nil => y::Nil
      case y::ys =>
        if (y == ys.head) then removeDupes1(ys)
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] =
    if(xs!=Nil) then countNums(xs.tail, xs.head, 1)
    else Nil

  def countNums [X] (ys: List[X], num: X, count: Int): List[(Int, X)] = 
    ys match
      case Nil =>
        if (num != null) then (count, num)::Nil
        else Nil
      case z::zs =>
        if (z!=num) then (count, num)::countNums(zs,z,1)
        else countNums(zs, z, count+1)
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    (n,xs) match
      case (_,Nil) => (Nil,Nil)
      case (-1, j) => (Nil, j)
      case (n, c::cs) => val (a,b) = splitAt(n-1,cs)
        if (n>0) then (c::a,b)
        else (a,c::b)
  ...
  def allDistinct(xs: List[Int]): Boolean =
    xs match
      case Nil => true //allDistinct (Nil) == true
      case y::ys => 
        if (member(y,ys) == true) then false
        else allDistinct(ys)
